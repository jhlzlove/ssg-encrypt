...
```bash
cargo run -- --config encrypt.toml --input public
```

Use `--dry-run` to scan and validate without writing files.

## Configuration

Copy `encrypt.example.toml` to `encrypt.toml` (git-ignored via `*.secret.toml`) and adjust:
...
| `password_id_attribute` | yes | Attribute on the matched node holding the password alias (maps to `[passwords]`). |
...
| Key | Default | Description |
|---|---|---|
| `iterations` | `600000` | PBKDF2 iteration count. Minimum 100 000 (enforced by both CLI and browser runtime). Only increase. |

### `[output]`

| Key | Default | Description |
|---|---|---|
| `remove_source_attributes` | `true` | Remove the password-id and hint attributes after encryption. |
...
Redacts Atom/RSS feed entries that link to encrypted pages. Matching is **path-based** (scheme/host ignored), so localhost preview builds and production builds redact identically.

| Key | Default | Description |
|---|---|---|
| `enabled` | `false` | Enable feed redaction. |
| `paths` | `["**/atom.xml", "**/rss.xml", "**/feed.xml", "**/index.xml"]` | Glob patterns for feed files. |
| `strip_prefix` | `""` | Subpath the site lives under, e.g. `"/docs"` for `example.com/docs`. Root-deployed sites leave this empty. |
| `placeholder` | `"This post is encrypted; open the original page to unlock it."` | Replacement text for encrypted entries. |
| `base_url` | `""` | **Deprecated** — use `strip_prefix`. The host part was never honoured. |

> **Note:** planet-style feeds that aggregate external articles should be excluded via `scanner.exclude`.

### `[passwords]`

Map of password aliases to real passwords. The alias appears in your HTML; the real password lives **only** in this config file. Never put real passwords in article front-matter or commit them to a public repo.
